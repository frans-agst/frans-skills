...
- Read **all files in `/context`** and **`docs/adr/`** before starting any task.
...
- **Start every session** by running `/remember restore` — it reads `context/progress-tracker.md` and the rest of `context/`, then confirms where we are before doing anything.
