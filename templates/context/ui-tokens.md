...
Design tokens for [PROJECT NAME]. All colors, typography, spacing, and component values extracted from the design. Use these exact values throughout the codebase — never hardcode colors or use raw Tailwind color classes in components.

> **Source of truth:** `context/design-system.md` is the design's *why and whole*. This file is its **enforced subset** — the concrete `@theme` tokens `/implement` applies. `/design-system` generates the design (via ui-ux-pro-max) and syncs the values down into here. When `design-system.md` changes, this file is re-synced; don't hand-edit tokens out of step with it.
