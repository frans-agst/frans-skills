...
1. **Check if an MCP server or docs tool is configured** for that library (e.g. Context7 or a vendor MCP). If one is available — use it for real-time docs before falling back to general knowledge.
2. **Read this file** for project-specific patterns that override general library knowledge.
3. Only then fall back to training knowledge — and treat it as possibly outdated.

The order of authority is:

```
MCP server / docs tool (real-time docs) → This file (project rules) → General training knowledge
```
