...
Persist the reset note so the fresh session can find it: add it to **Session Notes** in `context/progress-tracker.md` (and record any genuine bug under **Known Issues**), then run `/remember save`. The note is salvage context, not secrets — keep credentials out of it.

```
Next steps:

1. Saved the reset note to context/progress-tracker.md (Session Notes) via /remember save
2. End this session completely
3. Start a fresh session
4. Begin with /remember restore
