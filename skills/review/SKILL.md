...
- The current feature in `context/build-plan.md` — its UI, Logic, and done-criteria — plus any decisions agreed in the `/grill-with-docs` session that preceded the build
- `context/glossary.md` for the project's language, and `docs/adr/` for decisions in this area
- The relevant context files — `context/architecture.md` (boundaries), `context/code-standards.md`, and the design system: `context/design-system.md` (the visual source of truth) with `context/ui-rules.md` / `context/ui-tokens.md` (its enforced subset)

If no build-plan entry exists, ask the developer to describe what the feature was supposed to do before reviewing. You cannot verify correctness without knowing what correct looks like.
...
- **Design system** — does it follow `context/design-system.md` and its enforced subset? Correct tokens, classes, and patterns? Any hardcoded values that should be variables, any raw color classes that should use the design system? (Accessibility and Web Interface Guidelines compliance are **not** your job here — `/design-review` runs next and audits those against the live guidelines. Don't duplicate it.)
